...
int main(int argc, const char *argv[])
{
  ...
  xd3_init_config(&xd3Config,
                  0
//                  | XD3_COMPLEVEL_1
                  | XD3_COMPLEVEL_9
                  | XD3_ADLER32
                 );
  xd3Config.winsize = 256; //WINDOW_SIZE;
  xd3Config.getblk  = readSourceBlock;
  result = xd3_config_stream(&xd3Stream,&xd3Config);
  if (result != 0)
  {
    fprintf(stderr,"Can not initialize xdelta encoder (error: %d)!\n",result);
    exit(1);
  }
  ...
  if (result != 0)
  {
    fprintf(stderr,"Can not initialize xdelta encoder source (error: %d)!\n",result);
    exit(1);
  }
  ...
  while ((n = getc(stdin)) != EOF)
  {
    ...
    do
    {
      result = xd3_encode_input(&xd3Stream);
      switch (result)
      {
        case XD3_INPUT:
          // input consumed -> do nothing
          break;
        case XD3_OUTPUT:
          // output available -> write data
//fprintf(stderr,"%s,%d: XD3_OUTPUT xd3Stream.avail_out=%d\n",__FILE__,__LINE__,xd3Stream.avail_out);
        ...
        case XD3_WINFINISH:
          break;
        default:
          fprintf(stderr,"Invalid result of xdelta encoder (result: %d)!\n",result);
          exit(1);
          break;
      }
    }
...
//fprintf(stderr,"%s,%d: XD3_OUTPUT xd3Stream.avail_out=%d\n",__FILE__,__LINE__,xd3Stream.avail_out);
  ...
  fprintf(stderr,"pack: in=%lu out=%lu\n",in,out);
